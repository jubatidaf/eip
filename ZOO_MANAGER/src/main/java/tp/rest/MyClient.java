...
public class MyClient {
    ...
    public static void main(String args[]) throws Exception {
        ...
        //client.get_animals_By_Id().ifPresent(center -> System.out.println(center.toString()));
    ...
    //animal by id
    public Optional<Center> get_animals_By_Name(){
        return processResponse(
                restTemplate.getForEntity(url + "/animals/findByName/{name}", Center.class),
                () -> "Got an HTTP "+ HTTP_ERROR_CODE + " exception while listing the animals"
        );
    }
    
    public Optional<Center> get_cage_By_Name(){
        return processResponse(
                restTemplate.getForEntity(url + "/cage/findByName/{name}", Center.class),
                () -> "Got an HTTP "+ HTTP_ERROR_CODE + " exception while listing the animals"
        );
    }
